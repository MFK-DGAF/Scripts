...
function MIGRATE_FILE ([string]$File, [string]$DestDir, [string]$ServerName = $env:COMPUTERNAME)
{
...
function GET_SERVER_LIST ([string]$ServerName = $env:COMPUTERNAME, [string]$DBName = "Maintenance_Support", [string]$Domain = "")
{
...
$List_Servers_Query= 
@"
Select ServerName as Name from [Scripts].[Server] where domain like `$(DomainName)
"@

$Param = "DomainName='%" + $Domain + "%'"
$Results = invoke-sqlcmd -ServerInstance $ServerName -Database $DBName -Query $List_Servers_Query -Variable $Param
