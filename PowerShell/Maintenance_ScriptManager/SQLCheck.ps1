...
# 03/06/2017
# Changes: 
# Added a flag for the exception lists
# Added a section to pull the data from the exception file, and filter out those databases
#      from the Queries that check backup status
# Added the exceoption list and offline filter to full backup section
##########################################################################################
...
## Check if the exception list file exists and populates var with it's contents
############################################################################################
if (Test-Path $exceptionlist)
{
    $exceptionlist = get-content $ExceptionList
}

if ($ExceptionList -eq $null)
{
    $ExceptionList = ""
}
...
if ($differential)
{
    ...
    $MostRecentDiff = dir SQLSERVER:\SQL\localhost\DEFAULT\DATABASES | SELECT Name, LastDifferentialBackupDate, Status | Where {$_.Status -ne "Offline"} | where {$exceptionlist -notcontains $_.Name}
...
$MostRecent = dir SQLSERVER:\SQL\localhost\DEFAULT\DATABASES | SELECT Name, LastBackupDate, Status | Where {$_.Status -ne "Offline"} | where {$exceptionlist -notcontains $_.Name}
$JobStatus = "OK"
