param([string]$ParentBackDir = "F:\Backups", [string]$outputDir = "D:\HealthCheck", $WarnThreshold = .20, $ErrrorThreshold = .10 )

$ServerDrives = Get-WmiObject Win32_LogicalDisk
$ServerStatus = "OK"
$ServerMessage = ""
$ServerDriveMessage = ""
$ServerFile = write-output ($outputDir + "\Disk-" + $env:COMPUTERNAME + ".txt")
...
{	
	...
	if ($Drive.DriveType -eq 3)
	{
	    ...
	    if ($DrivePercentFree -lt $WarnThreshold)
	    {
	        ...
	        $DriveMessage = write-output ($Drive.DeviceID + " only has " + $red + $DriveFreeSpace + $end + "GB free`t<br>") 
...
if (($ServerStatus -eq "OK") -and ($backupcleanup_ran -eq 1)) 
{
    ...
    $ServerDriveMessage = write-output ($ServerDriveMessage) ("The Cleanup job was run on the server. It did not have sufficient space to run a full backup. <br>")
}

if ($ServerStatus -eq "OK"){ $ServerMessage = write-output ("Disk Space Check<br>") ("Status:              " + $ServerStatusText)}
if ($ServerStatus -ne "OK"){ $ServerMessage = write-output ("Disk Space Check<br>") ("Status:          " + $ServerStatusText) $ServerDriveMessage}
